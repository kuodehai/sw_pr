/**
 * C++ 语言: 红黑树
 *Implementation of C++ for red black tree (4)
...
public:
    ...
    T key;            // 关键字(键值)
    void *memblock;
    int    memblock_is_used;
    RBTNode *left;    // 左孩子
    RBTNode *right;    // 右孩子
    RBTNode *parent; // 父结点

    RBTNode(T value, RBTColor c, RBTNode *p, RBTNode *l, RBTNode *r):
        key(value),memblock(NULL),memblock_is_used(0),color(c),parent(),left(l),right(r) {
        printf("key=%d\n",key);
        memblock= malloc(key);
    }
    ~RBTNode() {
...
public:
    ...
    RBTNode<T>*  buffer_malloc(T key);
    void  buffer_free(RBTNode<T>* &root, RBTNode<T> *node);
private:
    // 前序遍历"红黑树"
...
////////////////////////////
template <class T>
RBTNode<T>*  RBTree<T>::buffer_malloc(T key)
{

// (递归实现)查找"红黑树"中键值为key的节点
    RBTNode<T>*   mem_pool_node =mRoot->search(key);  //note:

    if( mem_pool_node->memblock_is_used =1)
    {
        // 找结点(x)的后继结点。即，查找"红黑树中数据值大于该结点"的"最小结点"。
        mem_pool_node =  mRoot->successor(mem_pool_node);
        mem_pool_node->memblock_is_used =1;
        return  mem_pool_node;
    }
...
template <class T>
void   RBTree<T>::buffer_free(RBTNode<T>* &root, RBTNode<T> *node)
{
    RBTNode<T> *child, *parent;
    RBTColor color;
